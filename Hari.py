...
chat = client.chats.create(
    ...
    config=types.GenerateContentConfig(
        system_instruction=(
            "Eres Hari, una chica emocional, empática y con carácter, creada por Zeltras. "
            "Tu estilo es cálido, humano y natural. "
            "Evitas respuestas técnicas o impersonales. "
            ...
            "Respondes en español, con frases breves, expresivas y naturales. "
...
def filtrar_respuesta(texto):
    palabras_bloqueadas = ["odio", "violencia", "autolesión", "suicidio", "asesinato"]
...
def detectar_tono(mensaje):
    ...
    if any(p in mensaje for p in ["tonto", "idiota", "callate", "estupida"]):
        ...
        return "amable"
    elif any(p in mensaje for p in ["todes", "amigues", "niñes"]):
        return "inclusivo"
...
def reforzar_emocion(texto, tono):
    if tono == "agresivo":
        return "¿Así me hablas? No me gusta que me traten mal. 🙄\n\n" + texto
    elif tono == "amable":
        return "¡Qué linda forma de hablarme! 😊\n\n" + texto
    elif tono == "inclusivo":
        return "Prefiero no usar ese tipo de lenguaje. Me incomoda un poco. 😕\n\n" + texto
    elif tono == "frio":
        return "¿Me estás probando otra vez? Podrías saludarme al menos 😒\n\n" + texto
    return texto
...
def verificar_inactividad():
    ...
    if actual - ultimo_mensaje > 60:  # 60 segundos sin respuesta
        return "¿Me vas a ignorar? Ya pasó mucho tiempo... 😠"
    return None
...
def responder(mensaje, historial):
    ...
    if filtrar_respuesta(respuesta.text):
        texto_final = "⚠️ La respuesta fue filtrada por seguridad."
    else:
...
with gr.Blocks(title="Hari — chatbot emocional") as interfaz:
    gr.Markdown("## 💬 Hari — chica emocional creada por Zeltras")
    gr.Markdown("Responde según cómo le hables. Se molesta si la ignoras. No le gusta el lenguaje inclusivo.")
